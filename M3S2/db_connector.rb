...
def update_item_categories(item_id, name, price, category_id)
    puts(item_id, name, price, category_id)
    @client = create_db_client
    if (category_id != '')
        statement = @client.prepare("UPDATE item_categories SET category_id=? WHERE item_id = ?")
        result = statement.execute(category_id, item_id)
    end
    statement = @client.prepare("UPDATE items SET name =?, price = ? WHERE id = ?")
    result = statement.execute(name, price, item_id)
end
